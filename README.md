...
#### 缓存时间（天）
- **默认值**：`7`
- **说明**：拉取到的元数据（简介、图片、相似艺术家、歌曲 ID、歌词等）缓存多少天后重新获取
...
- **翻译歌词**：无独立开关。原文作为第一条下发；网易云有中文翻译时，翻译作为第二条（`lang=zh`）**始终随下**，由客户端（展示方）自行决定显示哪一条
...
- **缓存**：歌曲 ID 与歌词均按"缓存时间（天）"缓存；"未找到歌词"同样缓存，避免反复请求
